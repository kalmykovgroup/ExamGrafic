...
bool User::move(vector<Coordinate*>& path)
{
     
    Coordinate* end = path[path.size() - 1];
    path.erase(path.end() - 1);

    if (!checkingMove(this->getLabel(), path, end)) { //Проверяем что такой ход возможен и если возможен записываем в массив движение по клеткам            
